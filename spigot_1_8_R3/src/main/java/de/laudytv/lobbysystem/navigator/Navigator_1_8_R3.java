...
public class Navigator_1_8_R3 implements NavigatorMatcher {
    ...
    public void initializeItems(Inventory inventory, List<String> ids) {
        for (String id : ids) {
            ...
            if (plugin.warpData.getForceGlowing(id))
                itemStack = new ItemBuilder_1_8_R3(material)
    ...
    public void getNavigatorItem(Player player) {
        ...
        if (plugin.warpData.getSettingsGlowing("item"))
            itemStack = new ItemBuilder_1_8_R3(plugin.warpData.getSettingsMaterial("item"))
                    .setDisplayname(displayName)
