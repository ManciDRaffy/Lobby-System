...
public class ServerSelectorGUI_1_16_R3 {

    private final static LobbySystem plugin = LobbySystem.getPlugin();
    private final Inventory inv;
    private final HashMap<Integer, ItemStack> invItems = new HashMap<>();
    private final Player viewer;

    public ServerSelectorGUI_1_16_R3(Player viewer) {
    ...
    public void initializeItems() {
        ...
        for (String id : ids) {
            ...
            String serverOfPlayer = LobbySystem.getPlugin().getPluginMessage().getServer(viewer);
            if (plugin.ssData.getForceGlowing(id) || Objects.equals(server, serverOfPlayer))
                itemStack = new ItemBuilder_1_16_R3(material).setDisplayname(displayName).setLocalizedName(id).setLore(loreList.toArray(new String[0])).setGlowing().build();
            else
                itemStack = new ItemBuilder_1_16_R3(material).setDisplayname(displayName).setLocalizedName(id).setLore(loreList.toArray(new String[0])).build();
